...
const Picture: React.FC<ImageProps> = (props) => {
  const { onOk, onCancel, isSaveImageWithFullPath, open } = props;
  const [
    { fileList, loading, previewOpen, previewTitle, previewImage },
    setState,
  ...
  const handleOk = () => {
    const urls = fileList.map(({ response }) => {
      ...
      const tempUrl = isSaveImageWithFullPath
        ? `${host}${response.fileUrl}`
        : response.fileUrl;
      return {
  ...
  const renderAddImage = () => {
    ...
    return (
      <>
        ...
        {fileList.length === 0 && (
          <div className="editor-picture-description">
            支持 JPG、JPEG、PNG 等格式
          </div>
        )}
      </>
  ...
  return (
    <Modal
      title="上传图片"
      wrapClassName={
        fileList.length === 0 ? "editor-picture-wrapper-empty" : ""
      }
      open={open}
      onOk={handleOk}
