...
*/
class InsetCustomImgCommand extends Command {
  ...
  public execute({ src }: { src: string }): void {
    const model = this.editor.model;
    ...
    model.change((writer) => {
      const element = writer.createElement("customImage", { src });
  ...
  init() {
    ...
    editor.ui.componentFactory.add("customImage", (locale) => {
      ...
      buttonView.on("execute", () => {
        editor.execute("insetCustomImg", { src: 11 });
      });
